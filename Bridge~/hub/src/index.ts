...
function writeHubJson(port: number): void {
...
function deleteHubJson(): void {
  try {
    if (fs.existsSync(HUB_JSON_PATH)) fs.unlinkSync(HUB_JSON_PATH);
  } catch {
    // best effort
  }
}
...
async function main(): Promise<void> {
    ...
    for (const file of files) {
      try {
        ...
        process.stderr.write(
          `[hades-hub] Loaded pending: ${data.projectName}\n`
        );
      } catch {
  ...
  setInterval(() => {
    if (hub.registry.isIdle(AUTO_EXIT_MS)) {
      process.stderr.write(
        "[hades-hub] No instances or launcher activity for 60s, exiting.\n"
      );
      hub.close().then(() => {
        deleteHubJson();
        process.exit(0);
