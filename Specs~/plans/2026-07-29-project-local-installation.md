...
> **Follow-up 2 (post-Task 20).** Two further problems with "`.mcp.json` needs no change", both found by asking whether the file could be committed.
>
> 1. **It was overwritten, not merged.** `WriteProjectMcpJson` built a fresh single-entry object every server start, so any other MCP server a team declared in `.mcp.json` was silently deleted each time Unity came up. `.mcp.json` is Claude Code's project-level MCP registry, not a Hades-owned file. Now `MCPClientConfig.MergeHadesServer` sets only `mcpServers.hades` and preserves siblings and top-level keys; unparseable input is still replaced so Hades can self-heal its own entry. The shared `SetHadesServer` accessor also fixed a latent crash in the Desktop writer, where `"mcpServers": null` passed the `== null` guard as a JSON-null `JValue` and then failed a blind `(JObject)` cast.
> 2. **The absolute fallback above was avoidable, not inherent.** It only existed because the launcher copy was stored *inside* the resolved hub dir, so global hub scope moved it under `$HOME`. But the launcher never derives its hub from its own location — `resolveHubDir` (`Bridge~/launcher/src/hub-dir.ts`) reads `HADES_HUB_DIR`, walks up from cwd, then reads `hub_scope` from `config.local.yaml`, all at startup. The hub dir was serving as a storage location for a file that needed nothing but a version-stable path. `HadesPaths.LauncherDir` is now project-local in every scope, `HadesPaths.HubDir` keeps its scope-dependent meaning, and `hub-path.json` follows `HubDir` because `findHubEntry` reads it from there. `args[0]` is therefore identical on every machine under either scope, `.mcp.json` became git-tracked, and the `McpLauncherArg` absolute branch is retained only as a total function. Covered by `HadesPathsTests.LauncherDir_IsProjectLocal_ByConstruction` and `GlobalScope_MovesTheHub_ButNotTheLauncherArg`.
