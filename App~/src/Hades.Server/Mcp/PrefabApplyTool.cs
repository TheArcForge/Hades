...
/// call to <see cref="Hades.Tools.PrefabApplyCommands"/> (Plugin~), which applies every operation
...
/// ambiguous rather than merely inconsistent - see PrefabApplyCommands' own doc comment (Plugin~)
/// for the fuller rationale. This is an APP-LAYER rename only: the wire calls
/// <see cref="Hades.Tools.PrefabApplyCommands"/> (Plugin~) makes are byte-for-byte unchanged (still
...
/// </summary>
[McpServerToolType]
public sealed class PrefabApplyTool(EditorProxy editor)
{
        ...
        [Description("Project handle from hades_status. Omit when Hades knows only one project.")] string? project = null)
    {
        ...
        var result = await editor.SendCommandAsync(project, "prefab.apply", @params).ConfigureAwait(false);
    ...
    static WireJson BuildOperation(PrefabApplyOperation op)
    {
        ...
        // The plugin's wire contract (Plugin~/PrefabApplyCommands.cs, unchanged) still expects
