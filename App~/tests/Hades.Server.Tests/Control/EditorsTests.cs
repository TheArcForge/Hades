...
/// App~ can never call EditorApplication.Lock/UnlockReloadAssemblies at all (this assembly has no
/// UnityEditor reference), so the first test here is trivially, permanently true - it exists to
/// PIN that, not to catch a currently-real risk. The second test is Plugin~'s own
/// ReloadGateCriticalSuite.SourceScan_NoCallSitesForTheNativeLockApi_OutsideIEditorLockApi,
/// re-run here against the SAME real Plugin~ source tree (never modified - see this class's own
...
public sealed class EditorsSourceScanTests
{
    ...
    public void PluginSourceTree_StillHasExactlyTheTwoCallSitesInsideIEditorLockApi_NothingElse()
    {
        var toolsDir = PluginRequiredFields.FindPluginToolsDirectory() ?? throw new InvalidOperationException(
            "Could not locate Plugin~/Assets/Hades/Tools - see PluginRequiredFields' own doc comment. "
            + "Is Plugin~ present alongside App~ in this checkout?");
        var pluginRoot = Path.GetDirectoryName(toolsDir)!; // .../Plugin~/Assets/Hades
    ...
    static string FindAppSrcDirectory()
    {
        ...
        while (dir != null)
        {
            var candidate = Path.Combine(dir.FullName, "App~", "src");
            if (Directory.Exists(candidate) && Directory.Exists(Path.Combine(dir.FullName, "Plugin~")))
                return candidate;
            dir = dir.Parent;
        }
        throw new InvalidOperationException("Could not locate App~/src by walking up from " + AppContext.BaseDirectory);
    }
}
