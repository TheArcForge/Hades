...
1. **Standalone app (v2, the default today):** the SQLite file is in Hades' app storage, not the project folder: `~/Library/Application Support/Hades/projects/<productGUID>/traces.db`. The `project` handle `hades_status` returns for this project is that productGUID.
