...
namespace Hades.Tests.Editor
{
    ...
    /// (App~/src/Hades.Server/Mcp), a separate .NET process this netstandard2.1 Editor plugin never
    /// loads or references. Plugin~ EditMode tests also run inside whatever Unity project this plugin
    /// is deployed into, which is not guaranteed to have the rest of the Hades repo (App~, docs/)
    /// sitting alongside it on disk - so, unlike App~/tests/Hades.Server.Tests/PluginRequiredFields.cs
    /// (which parses Plugin~ source live, something only possible from that side because App~ tests
    ...
    ///     "Step 3: Measure";
    ///   - grep -rhoE '\[McpServerTool\(Name = "[a-zA-Z_]+"' App~/src/Hades.Server/Mcp/*.cs | sort -u
    /// </para>
    /// </summary>
    ...
    /// NOT re-verify the 32 names against App~/src/Hades.Server/Mcp (not reachable from here - see
