...
/// <summary>
/// Mines UnityPlugin/Assets/Hades/Tools/*.cs, at test-run time, for every <c>JsonParams.RequireString</c>/
...
/// UnityPlugin - which is how the live defect this mechanism exists to prevent happened in the first
...
internal static class PluginRequiredFields
{
    static readonly Lazy<IReadOnlyDictionary<string, IReadOnlyList<string>>> RealPluginTable = new(() =>
    {
        var toolsDir = FindPluginToolsDirectory() ?? throw new InvalidOperationException(
            "Could not locate UnityPlugin/Assets/Hades/Tools by walking up from " + AppContext.BaseDirectory
            + " - PluginWireContract's fake-Unity enforcement (EditorToolTestBase) and "
            + "PluginWireContractTests.cs both depend on it. Is UnityPlugin present alongside Core in this "
            + "checkout?");
        return Parse(toolsDir);
    });

    /// <summary>Every context found in the REAL UnityPlugin source, right now, mapped to the field
    ...
    public static IReadOnlyList<string> RequiredFieldsFor(string context)
    {
        ...
        throw new InvalidOperationException(
            $"No JsonParams.RequireString/RequireInt call was found in UnityPlugin/Assets/Hades/Tools/*.cs "
    ...
    /// has BOTH an 'Core' sibling and a 'UnityPlugin/Assets/Hades/Tools' sibling - i.e. the repo root -
    ...
    internal static string? FindPluginToolsDirectory()
    {
        ...
        while (dir != null)
        {
            var candidate = Path.Combine(dir.FullName, "UnityPlugin", "Assets", "Hades", "Tools");
            if (Directory.Exists(candidate) && Directory.Exists(Path.Combine(dir.FullName, "Core")))
                return candidate;
            dir = dir.Parent;
        }
        return null;
    ...
    // call in UnityPlugin/Tools passes a simple local variable here, never a nested expression.
    ...
    // Every 'ctx'-variable indirection in UnityPlugin/Tools follows this exact local-const shape
    ...
    /// UnityPlugin - so a test can point it at a small, hand-written scratch fixture to prove the parser
