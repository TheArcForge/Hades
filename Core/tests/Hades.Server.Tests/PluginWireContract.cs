...
/// context requires, parsed live from UnityPlugin source; this class supplies the STRUCTURE connecting
...
///
/// Verified against UnityPlugin/Assets/Hades/Tools/{Scene,Prefab,Material,Animation,Asset,SceneManagement,
            ...
            foreach (var field in PluginRequiredFields.RequiredFieldsFor(context))
            {
                if (!HasNonBlankValue(opJson, field))
                {
                    throw new InvalidOperationException(
                        ...
                        + "(JsonParams.RequireString/RequireInt in UnityPlugin/Assets/Hades/Tools, parsed live "
