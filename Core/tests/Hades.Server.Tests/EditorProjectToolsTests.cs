...
public sealed class EditorProjectToolsTests(WebApplicationFactory<Program> factory) : EditorToolTestBase(factory)
{
    ...
    {
        // F12 was "PlayMode runs silently save the open scene to disk" - traced to UnityPlugin's own
        ...
        // ProjectCommands.RunTests' own doc comment, UnityPlugin, for the full account). Fixed by
    ...
    // plugin-side only (see ProjectCommandsTests in UnityPlugin/Tests/Editor).
    ...
    // in UnityPlugin/Tests/Editor/ScriptEditingSessionTests.cs, dispatched there by WIRE method name
    ...
    // itself executed (UnityPlugin's CommandTable.Dispatch offering each one to
    ...
    // exactly as it did before: dispatched by UnityPlugin wire method name through the attached Editor,
