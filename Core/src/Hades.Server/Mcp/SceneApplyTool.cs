...
/// <see cref="Hades.Tools.SceneApplyCommands"/> (UnityPlugin) - see that class's own doc comment for how
...
/// "refused, not ignored" convention CommandTable.RejectUnknownParams (UnityPlugin) established for an
...
public sealed class SceneApplyTool(EditorProxy editor, ProjectService projects)
{
    ...
    /// ValidOps (UnityPlugin) - the two lists exist independently (this one runs before any wire call;
    ...
    /// the wire - see <see cref="Hades.Tools.SceneApplyCommands"/> (UnityPlugin), which accepts this
