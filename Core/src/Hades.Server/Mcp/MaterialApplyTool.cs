...
/// (UnityPlugin), which applies every operation directly inside one handler body, one Undo group - see
...
/// (UnityPlugin) makes are byte-for-byte unchanged (still 'path' for create, 'materialPath' for
...
public sealed class MaterialApplyTool(EditorProxy editor, ProjectService projects)
{
    ...
    static WireJson BuildOperation(MaterialApplyOperation op)
    {
        ...
        // The plugin's wire contract (UnityPlugin/MaterialApplyCommands.cs, unchanged) still expects
