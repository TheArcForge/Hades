...
/// editor-routed.json - Method is a UnityPlugin wire method name (e.g. "scene.create_gameobject",
...
/// than a single tool call should block for - see UnityPlugin's ProjectCommands.cs for the
...
/// description) and UnityPlugin's ProjectCommands.cs (ConsoleLogBuffer/TestRunResultStore) for why
...
/// offered every wire call it handled to a session held in UnityPlugin's ProjectCommands - which meant
...
/// including the shipped editor-routed.json - still dispatches by UnityPlugin wire method name through
...
/// behavior. That was wrong on two counts: the write was traced to UnityPlugin's own RunTests, which
...
/// ProjectCommands.RunTests' own doc comment (UnityPlugin) for the full account. The save was removed
...
/// ProjectCommandsTests' own AssertRunTestsNeverSavesDirtyScene (UnityPlugin) for the pins.</para>
/// </summary>
[McpServerToolType]
public sealed class EditorProjectTools(EditorProxy editor, ProjectService projects, LeaseRegistry leases, RegressionRecorder recorder)
{
    ...
    [Description("Records or replays Hades tool calls made in this session - not just what the "
               ...
               + "'scene.create_gameobject' - is dispatched by its UnityPlugin wire method name instead "
    ...
    /// handler already loops these internally (see its own doc comment on the UnityPlugin side), so
