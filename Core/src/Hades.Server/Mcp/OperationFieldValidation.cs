...
/// the inbound counterpart to <c>CommandTable.RejectUnknownParams</c> (UnityPlugin), which already
...
/// PluginRequiredFields.cs's own doc comment describes (Core/tests) - but that mechanism and this one
/// are deliberately separate, not shared code: PluginRequiredFields mines the PLUGIN's own source
/// (UnityPlugin/Assets/Hades/Tools/*.cs, via regex over JsonParams.RequireString/RequireInt calls) for
