...
/// it is `ProjectsViewModel.projects`' existing `indexState`/`indexStatus` fields, rendered verbatim
/// exactly like `ProjectsView` renders them.
///
/// **This step drives its own refresh.** An earlier version of this comment claimed those fields were
/// "already polled" - they are not, here. `MainWindowViewModel.refreshSelectedSection` drives the
/// ~1Hz tick for the main window; the onboarding window has no equivalent, so nothing re-read
/// `projects` and a successful add left "No Projects Yet" on screen. The `.task` below supplies that
/// tick for as long as this step is visible, and the server's own failure text is rendered above the
/// list the same way `ProjectsView` renders it - so "did that work?" is answerable either way.
///
...
/// codebase yet" (confirmed again for this task - `Core/src/Hades.Server/Control/SettingsEndpoint.cs`
...
struct OnboardingProjectsStepView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            // lastActionMessage`'s own doc comment. This is what a directory that is not a Unity
            // project reports; without it the picker simply closed and nothing changed.
        ...
        .task {
            // Supplies the tick the onboarding window otherwise lacks - see this type's own doc
            // comment. Cancelled automatically when the step goes away, so it costs nothing once
            // onboarding is done.
            while !Task.isCancelled {
                await viewModel.projectsViewModel.refresh()
