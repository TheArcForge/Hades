...
public final class ProjectsViewModel {
    ...
    /// the new row appearing in `projects` IS the feedback, so no Swift-invented success text is
    /// recorded into `lastActionMessage`.
    ///
    /// **It refreshes explicitly rather than waiting for a tick.** This previously relied on "the
    /// next `refresh()` tick", which is true only where something drives one:
    /// `MainWindowViewModel.refreshSelectedSection` does, while Projects is selected. The ONBOARDING
    /// window drives no tick at all, so an add there completed server-side and left "No Projects Yet"
    /// on screen - the success was real and entirely invisible. Refreshing here makes the feedback a
    /// property of the action instead of a property of whoever happens to be polling.
    public func addProject(path: String) async {
        guard let connection = await discover() else { return }
        do {
            _ = try await makeClient(connection).addProject(path: path)
            // A previous failure's text must not outlive the success that replaced it - otherwise a
            // "not a Unity project" error stays on screen above the row that just added fine.
