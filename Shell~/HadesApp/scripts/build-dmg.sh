...
# follows the same standard App~/src/Hades.Server/Mcp/McpBinding.cs holds for a port conflict
...
#
# Output:
#   Shell~/HadesApp/DerivedData/dmg/Hades-<version>.dmg           (signed path)
#   Shell~/HadesApp/DerivedData/dmg/Hades-<version>-unsigned.dmg  (--allow-unsigned path)
...
# build-app.sh's xcodebuild -scheme resolution requires cwd == Shell~/HadesApp (see that script's
