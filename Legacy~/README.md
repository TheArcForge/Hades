# Legacy~ — retired v1.2 delivery files

`claude-plugin/plugin.json` used to sit at the repository root as `.claude-plugin/plugin.json`,
where Claude Code treats it as an **installable plugin manifest**. It described the v1.2 product —
`"90 MCP tools"`, version `1.2.0` — and is kept here for reference only.
...
`Plugin-ClaudeCode~/` is the current Claude Code plugin. It carries the same skills and commands,
...
`V12Cleanup` in `App~/src/Hades.Core/Migration/` read the *user's* machine — their `CLAUDE.md`,
...
come back. See `App~/tests/Hades.Server.Tests/ClaudeCodePluginTests.cs`.
